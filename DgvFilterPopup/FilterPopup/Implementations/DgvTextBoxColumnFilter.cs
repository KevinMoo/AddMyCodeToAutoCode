    ...
    public partial class DgvTextBoxColumnFilter : DgvBaseColumnFilter {
        ...
        protected override void OnFilterInitializing(object sender, CancelEventArgs e) {
            ...
            if (ColumnDataType == typeof(string))
                comboBoxOperator.Items.AddRange (new object[] { "..xxx..","xxx..","..xxx" , "=", "<>", "=Пе", "<>Пе" });
            else
                comboBoxOperator.Items.AddRange(new object[] { "=", "<>", ">", "<", "<=", ">=", "=Пе", "<>Пе" });
        ...
        protected override void OnFilterExpressionBuilding(object sender, CancelEventArgs e){
            ...
            if (comboBoxOperator.Text == "=Пе") ResultFilterExpression = GetNullCondition(this.DataGridViewColumn.DataPropertyName);
            if (comboBoxOperator.Text == "<>Пе") ResultFilterExpression = GetNotNullCondition(this.DataGridViewColumn.DataPropertyName);
